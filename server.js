...
manager
     .train()
     .then(async () => {
          manager.addDocument("en", "user.data", "give me records of the database");
          manager.addAnswer("en", "user.data", "please wait i am working on it...");
          manager.save();

          express.post("/use-bot", async (req, res) => {
               const message = req.body.question;
               if (!message) {
                    return res.json({
                         error: "please type message",
                    });
               }
               var response = await manager.process("en", message);
               return res.json({
                    botAnswer: response.answers.length ? response.answer : "ANSWER_NOT_EXIST",
                    timeZone: new Date(),
                    user: "BOT",
               });
          });
